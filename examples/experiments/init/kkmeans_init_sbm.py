...
def perform_graph(graph, kernel_class: Type[Kernel], estimator: KKMeans, n_params=51):
    (A, y_true), G = graph
    ...
    for param_flat in np.linspace(0, 1, n_params):
        ...
        try:
            ...
            print(f'{kernel.name}, {param_flat}: {e}')
...
def perform_kernel(column, graphs, kernel_class, n_params=51, n_jobs=6, n_gpu=2,
    ...
    def _calc(n_graphs=None, n_params=n_params, n_jobs=n_jobs):
        kmeans = partial(KKMeans, n_clusters=k, init='any', n_init=30, init_measure='modularity')
