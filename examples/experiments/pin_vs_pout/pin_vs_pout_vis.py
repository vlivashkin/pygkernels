...
def draw(plot_results: PlotResults, p_ins, p_outs,
    ...
    for pin_idx in range(side_len):
        for pout_idx in range(side_len):
            ...
            if np.sum(np.array([x[1] for x in measure_best_ari]) == 1) == 1:
    ...
    ax.set_xticklabels(p_outs, rotation=90)
    ax.set_ylabel('$p_{in}$')
    ...
    ax.set_yticks(range(0, len(p_ins) * 2, 2))  # * 2, 2
...
if __name__ == '__main__':
    ns, ks, estimators, pin_pout_step = [100], [2], [KKMeans], 0.05
