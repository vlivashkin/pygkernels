...
class StochasticBlockModel(GraphGenerator):
    def __init__(
        self, n_nodes, n_classes, cluster_sizes=None, balance=None, p_in=None, p_out=None, probability_matrix=None
    ):
        self.n = n_nodes
        self.k = n_classes
        ...
        if balance is not None:
            ...
            self.cluster_sizes = (
                [1] * self.k + (self.n - self.k) * softmax(np.arange(self.k)[::-1], beta=balance)
            ).astype(np.int)
    ...
    def generate_info(self, n_graphs):
        ...
        if not self.probability_matrix_mode:
            info.update({"p_in": self.p_in, "p_out": self.p_out})
        return info
